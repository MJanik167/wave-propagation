...
int main(int argc, char **argv)
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//timer(); // Update the simulation state/przeniesione niżej
		rysuj(); 
		...
		// 2. Show a simple window that we create ourselves. We use a Begin/End pair to create a named window.
		{
			...
			ImGui::Text("Color palette is created using the following formula color(t) = a + b * cos[ 2*pi(c*t+d)]\nwhere t is the pressure represented by each pixel\na,b,c,d parameters for each color are represented by the fields below (modifiable)");
